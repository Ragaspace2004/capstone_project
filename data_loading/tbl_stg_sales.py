...
# ============================================
# This script:
# 1. Reads data from multiple CSV files into a Pandas DataFrame
...
CSV_FILES = [
    r"AdventureWorks_Sales_2015.csv",
    r"AdventureWorks_Sales_2016.csv",
    r"AdventureWorks_Sales_2017.csv"
]  # List of CSV files to combine
TARGET_TABLE = "sales"                     # Table where data will be loaded
...
# ---------------------------------------------------
def main():
    # Step 1: Read CSV files into DataFrame
    # Using dtype=str ensures all data is treated as text (safe for mixed types)
    dfs = []
    for csv_file in CSV_FILES:
        df = pd.read_csv(csv_file, encoding="latin-1", dtype=str, na_filter=False)
        dfs.append(df)
    ...
    try:
        ...
        with engine.begin() as conn:
            ...
            INSERT INTO `{AUDIT_TABLE}` (
                ...
                '{PROCESS_NAME}', '{DATABASE}', '{TARGET_TABLE}', '{LOAD_MODE}', '{csv_paths}',
